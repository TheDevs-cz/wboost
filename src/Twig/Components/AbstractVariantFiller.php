...
abstract class AbstractVariantFiller extends AbstractController
{
    ...
    public function __construct(
        ...
        private readonly FillTextPlaceholders $fillTextPlaceholders,
    ...
    /**
     * Per-text-input data for the fill overlay (frame, rules, current value,
     * WYSIWYG seed, font/colour options) — see
     * {@see FillTextPlaceholders::placeholders()}, the ONE builder both fill
     * pages share. Mirrors {@see imagePlaceholders()}.
     *
    ...
    public function textPlaceholders(): array
    {
        ...
        return $this->fillTextPlaceholders->placeholders(
            $variant,
            $this->textValues,
            $this->hiddenValues,
            $this->fontValues,
    ...
    /**
     * Combined layers list for the "Vrstvy" panel — fillable placeholders
     * only, topmost first ({@see FillTextPlaceholders::layers()}).
     *
    ...
    public function layers(): array
    {
        ...
        return $this->fillTextPlaceholders->layers($variant, $this->hiddenValues);
    }
     ...
     * template then skips the WYSIWYG chrome entirely.
     *
     * @return null|array{
    ...
    {
        return $this->fillTextPlaceholders->richTextToolbar($this->variantEntity(), $this->fontOptions());
    }

    /**
     * The rich-text options, resolved whenever ANY fillable input can switch
     * fonts (rich, or a plain input the designer opened up) — the popovers'
     * face menus come from it and the whole-text font override is validated
     * against it in the render. Memoized: the resolver hits the fonts +
     * manuals queries and several render methods need it during one request.
     */
    private function fontOptions(): null|RichTextOptions
    {
        return $this->richTextOptionsCache ??= $this->fillTextPlaceholders->fontOptions($this->variantEntity());
    }
    ...
    /**
     * The overlay's client-side reflow payload (designed frames + text
     * metrics + containers) — {@see FillTextPlaceholders::layoutData()}.
     *
