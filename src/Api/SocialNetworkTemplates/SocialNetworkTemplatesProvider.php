            ...
            fn (SocialNetworkTemplate $template): SocialNetworkTemplateResponse => new SocialNetworkTemplateResponse(
                ...
                variants: array_values(array_map(
                    fn ($variant): SocialNetworkTemplateVariantResponse => new SocialNetworkTemplateVariantResponse(
                        ...
                        inputs: array_map(
                            fn (int $index): SocialNetworkTemplateVariantInputResponse => new SocialNetworkTemplateVariantInputResponse(
                                index: $index,
                                name: $variant->inputs[$index]->name,
                                maxLength: $variant->inputs[$index]->maxLength,
                                locked: $variant->inputs[$index]->locked,
                                uppercase: $variant->inputs[$index]->uppercase,
                                description: $variant->inputs[$index]->description,
                                hidable: $variant->inputs[$index]->hidable,
                            ),
                            array_keys($variant->inputs),
                        ),
                    ),
                    $template->variants(),
                )),
