...
/**
 * One gallery image a consumer may drop into a specific image placeholder. The
 * collection is scoped to the variant + placeholder and contains only images
 * from the folders the designer allowed for that slot (for an UNRESTRICTED
 * slot that is the whole gallery — root files come back with a null
 * `directoryId`/`directoryName`). Reference `id` in the export `images` map.
 */
#[ApiResource(
    shortName: 'PlaceholderGalleryImage',
    operations: [
        new GetCollection(
