...
namespace WBoost\Web\Tests\MessageHandler\CustomTemplate;
...
use WBoost\Web\MessageHandler\CustomTemplate\CopyCustomTemplateHandler;
use WBoost\Web\MessageHandler\CustomTemplate\CopyCustomTemplateVariantHandler;
 ...
 *
 * @covers \WBoost\Web\MessageHandler\CustomTemplate\CopyCustomTemplateHandler
 * @covers \WBoost\Web\MessageHandler\CustomTemplate\CopyCustomTemplateVariantHandler
 */
final class CopyCustomTemplateHandlersTest extends KernelTestCase
{
    public function testCopyTemplateCopiesVariantsWithImageInputs(): void
    {
        $newTemplateId = Uuid::uuid4();

        $handler = self::getContainer()->get(CopyCustomTemplateHandler::class);
        $handler(new CopyCustomTemplate(
        ...
        $copy = self::getContainer()->get(CustomTemplateRepository::class)->get($newTemplateId);
    ...
    public function testCopyVariantKeepsDimensionAndImageInputs(): void
    {
        ...
        $handler = self::getContainer()->get(CopyCustomTemplateVariantHandler::class);
        $handler(new CopyCustomTemplateVariant(
        ...
        $copy = self::getContainer()->get(CustomTemplateVariantRepository::class)->get($newVariantId);
